-- NMTC Core Engine Database Schema Application
-- Run this script directly in Supabase SQL Editor or via psql

-- Note: Run each section separately in Supabase SQL Editor if needed

-- 1. Create Custom Types
CREATE TYPE IF NOT EXISTS processing_session_status AS ENUM (
    'queued',
    'starting',
    'stage_0a_running',
...
CREATE TYPE IF NOT EXISTS agent_stage_status AS ENUM (
    'pending',
    'running',
    'completed',
    'failed',
...
-- 2. Create Processing Sessions Table
CREATE TABLE IF NOT EXISTS public.processing_sessions (
    id uuid NOT NULL DEFAULT gen_random_uuid(),
    document_id uuid NOT NULL,
    user_id uuid,
    org_id uuid NOT NULL,
    session_type text NOT NULL DEFAULT 'full_pipeline',
...
-- 3. Create Agent Pipeline States Table
CREATE TABLE IF NOT EXISTS public.agent_pipeline_states (
    id uuid NOT NULL DEFAULT gen_random_uuid(),
...
-- 4. Create Background Jobs Table  
CREATE TABLE IF NOT EXISTS public.background_jobs (
    ...
    CONSTRAINT timeout_range CHECK (timeout_seconds >= 30 AND timeout_seconds <= 7200)
);

-- 5. Create Progress Events Table
...
-- 7. Create Processing Errors Table
...
-- Create Performance Indexes (Run separately if needed)
-- Processing Sessions Indexes
CREATE INDEX IF NOT EXISTS idx_processing_sessions_document_id ON public.processing_sessions(document_id);
CREATE INDEX IF NOT EXISTS idx_processing_sessions_status ON public.processing_sessions(status);
CREATE INDEX IF NOT EXISTS idx_processing_sessions_org_id ON public.processing_sessions(org_id);
CREATE INDEX IF NOT EXISTS idx_processing_sessions_heartbeat ON public.processing_sessions(last_heartbeat);

-- Background Jobs Indexes (Critical for performance)
CREATE INDEX IF NOT EXISTS idx_background_jobs_status_priority ON public.background_jobs(status, priority) WHERE status IN ('queued', 'retry_scheduled');
CREATE INDEX IF NOT EXISTS idx_background_jobs_job_key ON public.background_jobs(job_key);

-- Progress Events Indexes (Real-time queries)
CREATE INDEX IF NOT EXISTS idx_progress_events_session_timestamp ON public.progress_events(session_id, timestamp DESC);
...
CREATE POLICY IF NOT EXISTS "Users can view processing sessions from their organization" ON public.processing_sessions
    FOR SELECT USING (
        org_id IN (
...
CREATE POLICY IF NOT EXISTS "Users can insert processing sessions in their organization" ON public.processing_sessions
...
COMMENT ON TABLE public.processing_sessions IS 'Enterprise processing sessions with heartbeat monitoring';
...
COMMENT ON TABLE public.user_sessions IS 'Cross-device session synchronization';
